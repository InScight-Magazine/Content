...
show heading.where(level: 2): it =>[
  ...
  #set text(font: alternate-font, weight: heading-weight, size: heading-2-size)
  #block(it.body)
]
...
if authorInfo != none {
    ...
    { 
      set text(size: 1.2em, font: header-font, weight: "medium")
      set par(leading: 0.2em, justify: false)
      align(left, eval(authorInfo, mode: "markup"))
    }
    image(authorImage)
    v(1fr)
    colbreak()
...
#let interview(
  ...
  let content = for textLine in lines {
    ...
    if boldflag == true {
      ...
      text(weight: "semibold", size:1.0em, font: alternate-font)[#trimmedLine]
    } else {
...
#let quiz(
  ...
  content = for (question, options, img) in questions.zip(options, images) {
    ...
    //     [+ #eval(line, mode: "markup")]
    //   },
    //     align(center, image(i, height:15em))
    //   )
    // } else {
