...
#let abstract = "*Wilson disease occurs due to toxic copper accumulation* and oxidative stress, yet current therapies only chelate excess copper but doesn't reduce oxidative stress. This study repurposes melatonin as a dual-action molecule that both chelates copper and modulate redox imbalance."
...
#img(path: "/images/pandey4.jpg", caption: "Due to excess copper accumulation, there is more oxidative stress in ATP7B$\"\"^\"-/-\"$ HepG2 cells.", portrait: true, position: bottom)
...
To study copper toxicity, we created a simple disease model by using CRISPR to delete ATP7B in a human liver cell line (HepG2). Because the liver is the main organ affected in Wilson disease and ATP7B is its key copper exporter, removing this gene causes copper to accumulate in HepG2 cells—closely mimicking the Wilson disease phenotype in ATP7B$""^"-/-"$\_HepG2. Using this model, we compared the copper toxicity of the normal liver cell line (WT_HepG2) with cells lacking ATP7B (ATP7B$""^"-/-"$\_HepG2). When both were exposed to copper, cell survival dropped in each case, but the ATP7B-deficient cells were far more sensitive. Without the ability to export copper, these cells accumulated higher copper levels and suffered more severe damage (Fig 3). 

We compare the oxidative stress level of ATP7B$""^"-/-"$\_HepG2 with WT_HepG2 cells, we found increased level of oxidative stress, which is the primary reason for the sharp decrease in cell viability ATP7B$""^"-/-"$\_HepG2, following the contemporary cellular model for Wilson Disease (Fig 4).
...
Although melatonin is remarkably safe, it has a major limitation as a drug: a short half-life in the bloodstream. To overcome this, we collaborated with chemists to design redox-responsive melatonin loaded polymeric nanocapsules (MNC) capable of carrying and protecting melatonin. Encapsulating melatonin transformed how it behaves in the body. The nanocapsules increased its stability. (Fig 15), enabling the drug to reach its target in a much more effective way. More importantly, these nanocapsules were engineered to be responsive to the cellular environment, when oxidative stress is high. When nanocapsules experienced high levels of intracellular glutathione-a signature of oxidative stress-they released melatonin exactly where and when needed (Fig 16).
This targeted release dramatically enhanced melatonin's effectiveness, up to almost a three-fold increase in scavenging reactive oxygen species (Fig 17). In doing so, nanotechnology transformed melatonin from a simple dietary supplement into a smart, responsive therapeutic bridge between chemistry and biology that can be used to improve the treatment potential of Wilson disease.
