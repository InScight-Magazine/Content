...
#let n_eq(content) = math.equation(
    block: true,
    numbering: none,
    content,
)
...
#n_eq($ Delta S_"total" = Delta S_"system" + Delta S_"environment" >= 0 $)
...
+ Attach a frictionless piston to the correct side. The molecule pushes the piston outward, expanding isothermally and doing work #n_eq($ W_"out" = k_B T ln 2 $)
...
#n_eq($ W_"erase" = k_B T ln 2 = W_"out" $)
...
#n_eq($ H = - sum_i p_i log_2 p_i $)

while in statistical physics, the Boltzmann entropy reads

#n_eq($ S = -k_B sum_i p_i ln p_i = k_B ln 2 H $)
