...
#let title = "Patterns In Primes Via Probability"
#let authors = ("Anup Dixit", "Siddhi Pathak",)
#let affiliations = ("IMSc, India", "CMI, India")
...
$ pi(N) \:= \#{ p <= N : p "is prime" } ~ N / (log N) "as" N -> oo. $
...
known as the _flogarithmic integral_. This approximation is surprisingly accurate: for example, there are exactly $78498$ primes below $10^6$, while $"li"(10^6) approx 78627$, an error of only about $0.16\%$. 
...
Using results from probability theory, Cram\'er showed that with probability one,
\[
\limsup_{n\to 00} frac(P_{n+1}-P_n}{(\log P_n)^2} = 1.
\]
...
Although powerful, Cram\'er’s model is undeniably simplistic. It ignores basic arithmetic constraints, such as the fact that all primes except $2$ are odd, or that no number divisible by $3$ can be prime. Consequently, relying on the model without modification can lead to misleading predictions. 

A striking example concerns twin primes—pairs of primes $(p,p+2)$. While it is widely believed that infinitely many twin primes exist, this remains unproven. Cram\'er’s model predicts that the number of twin primes up to $N$ should be approximately
...
Nevertheless, when refined to account for divisibility by small primes, Cram\'er’s framework yields more accurate predictions. Granville showed that such corrections replace the constant $1$ in the strong conjecture with $2e^{-\gamma}\approx1.229$, where $\gamma$ is Euler’s constant. Similarly, the refined model predicts that the number of twin primes up to $N$ should be
$ ~ 2 C_2 N / (log(N))^2 C_2 = product_(p text(is prime), p >= 3) (1 - 1 / (p - 1)^2) approx 0.66016$
known as the _logarithmic integral_. This approximation is surprisingly accurate: for example, there are exactly $78{,}498$ primes below $10^6$, while $\mathrm{li}(10^6) \approx 78{,}627$, an error of only about $0.16\%$. 
...
// Dividing the length of the interval by the number of primes it contains suggests that the _average_ gap between consecutive primes near $N$ is about $\log N$. This reasoning also reinforces Gauss’s original insight that roughly one out of every $\log N$ numbers near $N$ is prime. 
//
// This line of thought inspired the Swedish mathematician Harald Cram\'er in 1936 to introduce a probabilistic model for primes. Imagine an infinite sequence of urns $(U_n)_{n\in\bb{N}}$. The urn $U_1$ contains only blue balls, $U_2$ only red balls, and for $n\ge3$, the urn $U_n$ contains both colors. Suppose that when drawing a ball from $U_n$, the probability of obtaining a red ball is $1/\log n$. Drawing independently from each urn produces an infinite sequence of red and blue outcomes. Let $P_n$ denote the index of the urn from which the $n$-th red ball is drawn. The resulting sequence $(P_n)_{n\in\bb{N}}$ is increasing and serves as a model for the sequence of prime numbers. 
//
// If we define
// \[
// \Pi(N) := \#\{ n \in \bb{N} : P_n \le N \},
// \]
// then
//
// $ bb(E)[Pi(N)] = sum_(n=1)^N \frac(1, log n) approx mathrm(li)(N) approx frac(N, log N). $
//
// This mirrors the behavior of $\pi(N)$, reinforcing the analogy between $(P_n)$ and the primes. In particular, one may expect the largest gaps between primes to resemble the largest gaps between successive $P_n$. 
//
// Using results from probability theory, Cram\'er showed that with probability one,
...
// Let $2=p_1<p_2<p_3<\cdots$ be the sequence of prime numbers.
// \begin{enumerate}
//     \item[(a)] (Strong form) $\displaystyle \limsup_{n\to 00} frac(p_{n+1}-p_n}{(\log p_n)^2} = 1$.
//     \item[(b)] (Weak form) $\displaystyle p_{n+1}-p_n \ll (\log p_n)^2$.
// \end{enumerate}
// \end{conjecture}
//
// Although powerful, Cram\'er’s model is undeniably simplistic. It ignores basic arithmetic constraints, such as the fact that all primes except $2$ are odd, or that no number divisible by $3$ can be prime. Consequently, relying on the model without modification can lead to misleading predictions. 
...
//
// Nevertheless, when refined to account for divisibility by small primes, Cram\'er’s framework yields more accurate predictions. Granville showed that such corrections replace the constant $1$ in the strong conjecture with $2e^{-\gamma}\approx1.229$, where $\gamma$ is Euler’s constant. Similarly, the refined model predicts that the number of twin primes up to $N$ should be
// $ 2 C_2 frac(N, (log N)^2), C_2=prod_(p "prime", p ge 3) (1-frac(1, (p-1)^2))approx 0.66016. $
//
// Cram\'er’s model also sheds light on the distribution of primes in short intervals. Under this framework, intervals $[N,N+H)$ with $(\log N)^{2+\delta}<H<N$ typically contain about $H/\log N$ primes. In 1943, assuming the Riemann Hypothesis, A. Selberg showed that this holds for _almost all_ such intervals. For many years, it was believed that this behavior should hold uniformly. However, in a groundbreaking result, H. Maier demonstrated in 1985 that there are infinitely many short intervals where the number of primes deviates significantly from this expectation. 
//
// Seen in this light, Cram\'er’s model reveals both the power and the limitations of probabilistic reasoning in number theory. It captures the correct scale of prime gaps while falling short of fully encoding arithmetic structure. Its successes and failures together show why such models are best viewed as guiding principles rather than final answers. By exploring where the model works, where it breaks down, and how it can be refined, mathematicians continue to uncover subtle patterns beneath the surface, ensuring that the study of prime numbers remains a vibrant and evolving field.
