...
#let title = "Quantum at 100: From Heisenberg’s Revolution to Modern Science" 
...
#let authors = ("Anu Arora", "Sourav Pal")
...
#let coverImage = "/covers/sourav2.svg"
#let authorImage = "/authFaces/sourav.png"
// cover image and author image are kept in their respective folders, which must be reflected in the paths

#let received = (month: 04, day: 29, year: 2026)
// article finalising date

#let abstract = "What if the electron never truly orbited the nucleus, not because its path was too small to be
observed, but because the very idea of a definite path was the wrong question? A century ago,
quantum mechanics forced science to forsake the familiar language of classical motion and
rethink how nature itself should be described. This article traces the remarkable journey-
from Heisenberg’s rejection of electron orbits to the quantum theories of chemical bonding,
electron correlation, and modern computational science, revealing how one conceptual
revolution transformed our understanding of atoms, molecules and materials."
// String. Abstract/excerpt/pitch for the article. Formatting is possible and encouraged.

#let reviewedBy= ("Archita Sarkar", "Dishari Dasgupta")
...
#show: article.with(
  ...
  authorImageWidth: 90%,
  refsFile: "/dataFiles/sourav.yml",
  // breakAfter: (-1,),
)

#quote[What we observe is not nature itself, but nature exposed to our method of questioning. _— Werner Heisenberg_]
...
```
integral p d q = n h \/ 2π, 
```
[5] `(n = 1, 2, 3 dots)`. While this model successfully explained the hydrogen spectrum, it could not accurately describe fine spectral properties like anomalous Zeeman effect [6], Stark splitting [7], or transition intensities, indicating the absence of a theoretical foundation.
...
#quote[All of my pitiful efforts are directed at completely killing off the concept of orbits, which, after all, cannot be observed, and replacing it with something more suitable.]

In this new approach, physical quantities were represented through a transition between quantum states rather than through a definite electron path. The central aspect of the new formalism was that the ordering of operators is essential, such that `x y != y x` [8-9]. The formalism was later extended into matrix multiplication by Max Born and Pascual Jordan, who represented physical observables as matrices acting on quantum states. This new theory successfully explained atomic spectra, including discrete energy levels and transition rules.
...
#img(path: "/images/sourav-1.svg", caption: "Timeline of key milestones in quantum mechanics and computational chemistry, from Max Planck’s radiation quanta to modern approaches including Density Functional Theory and advanced post-Hartree–Fock approaches for atomic and molecular systems.")
...
H Psi = E Psi, 
```
where `H` is the Hamiltonian operator, `Psi` is a two-electron wavefunction, and `E` is the corresponding energy eigenvalue. For the hydrogen molecule, the full molecular Hamiltonian in atomic units (`planck  = m_e = e = 1`) takes the form: 
```
hat(H) = -1/2 nabla_1^2-1/2 nabla_2^2 - (1/r_(1A) + 1/r_(1B) + 1/r_(2A) + 1/r_(2B) ) + (1/r_12 + 1/r_(A B))
```
where `nabla_1^2` and `nabla_2^2` are the Laplacian operators representing the kinetic energy of electrons 1 and 2, respectively, `r_(1A)`, `r_(1B)`, `r_(2A)` and `r_(2B)` denote the distances of each electron from nuclei A and B, `r_12` is the interelectronic distance, and `R_(A B)` is the internuclear separation. The first two terms are kinetic energy, the next four terms are electron-nuclear attraction, and the last two terms are electron-electron and nuclear-nuclear repulsion,
...
- *Electronic part*: Solve for electrons in a fixed nuclear framework to obtain `E_e (R)`. 

- *Nuclear part*: Use `E_e (R)` as an effective potential governing nuclear motion. 
...
```
E[rho] = T_s [rho] + integral V_"ext" rho + E_H [rho] + E_"xc" [rho],
```
where, `T_s [rho]` is the kinetic energy of an auxiliary system of non-interacting electrons, `E_H [rho]` is the classical Hartree term describing electron–electron electrostatic repulsion, and `E_"xc" [rho]` is the exchange–correlation functional.

The corresponding Kohn–Sham equations take the form:
```
[- 1/2 nabla^2 + V_"eff" (r) ] phi_i (r) = epsilon_i phi_i (r),
```
where `V_"eff" (r)` is an effective potential and `phi_i (r)` is the `i`-th Kohn-Sham orbital and `epsilon_i` is the corresponding Kohn-Sham orbital energy eigenvalue. The electron density is therefore constructed from the orbitals as follows:
```
...
#img(path: "/images/sourav-2.png", caption: "Schematic representation of Jacob’s ladder in DFT, approximations from LDA → GGA → meta-GGA → hybrid → double hybrid, showing increasing accuracy.", position: top)
...
#img(path: "/images/sourav-3.png", caption: "Comparison of computational scaling with increasing system size for electronic-structure methods. The log of CPU time is plotted against the number of atoms `N`, showing characteristic polynomial scalings: semi-empirical methods (`-N^2`), DFT (`-N^4`), MP2 (`-N^5`), CCSD (`-N^6`), and CCSD(T) (`-N^7`). The figure highlights the rapidly increasing computational expense of higher-accuracy correlated methods with system size.", portrait: true, position: top)
